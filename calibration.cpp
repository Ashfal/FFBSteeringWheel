...
static uint16_t find_zero_pwm(Direction dir, MotorControl& motor, I2CDMA& i2c, AS5600Parser& parser) {
    uint16_t test_pwm = 0;
    ...
    while (test_pwm < PWM_WRAP) {
        ...
        if ((dir == Direction::CW && vel > 10) || (dir == Direction::CCW && vel < -10)) {
...
// Sweep at a constant PWM and find maximum stable speed
static int32_t measure_max_speed(uint16_t pwm, Direction dir, MotorControl& motor, I2CDMA& i2c, AS5600Parser& parser) {
    motor.set_target(pwm, dir, 0);
    ...
    while (true) {
        ...
        if (dir == Direction::CCW) distance = -distance;
...
void run_calibration(SharedState* state, I2CDMA& i2c, MotorControl& motor, AS5600Parser& parser) {
    ...
    // 1. Find Zero PWM
    luts.cw_zero_pwm = find_zero_pwm(Direction::CW, motor, i2c, parser);
    luts.ccw_zero_pwm = find_zero_pwm(Direction::CCW, motor, i2c, parser);
    ...
    // 2. Measure speeds at different PWM levels
    for (uint8_t i = 0; i < CAL_PWM_LEVEL_COUNT; i++) {
        uint16_t pwm = CAL_PWM_LEVELS[i];
        
        luts.cw_speed[i] = measure_max_speed(pwm, Direction::CW, motor, i2c, parser);
        luts.ccw_speed[i] = measure_max_speed(pwm, Direction::CCW, motor, i2c, parser);
    }
